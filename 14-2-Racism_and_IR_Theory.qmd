...
# Input WDI data and exclude aggregates
d <- read_excel("../../../../00-DATA_Teaching/World_Bank-World_Development_Indicators/WDIEXCEL-2026-01-26.xlsx", na = "NA")

d2 <- read_excel("../../../../00-DATA_Teaching/World_Bank-World_Development_Indicators/List_of_WDI_Economies_Excluding_Aggregates.xlsx", na = "NA")

d <- d |>
  filter(`Country Code` %in% d2$`Country Code`)
```
...
# Map current year of GDP per capita
# Select variable AND only the countries
...
q1_countries <- d_gdp |>
  filter(year == 1970) |>
  filter(gdp_pc_current_usd <= quantile(d_gdp$gdp_pc_current_usd[d_gdp$year == 1970], na.rm = T, probs = .25)) |>
  select(country)

q4_countries <- d_gdp |>
  filter(year == 1970) |>
  filter(gdp_pc_current_usd >= quantile(d_gdp$gdp_pc_current_usd[d_gdp$year == 1970], na.rm = T, probs = .75)) |>
  select(country)
...
```{r, fig.align='center'}
knitr::include_graphics("Images/14-1-fragility_world_map2026.png")
```
