...
# If pulled in long format from WDI
# Rename columns in spreadsheet and remove the bottom two rows
d <- read_excel("../../Data/WDI/GDP_pc-1970-2024.xlsx", na = "NA")

# # WDI database not working so downloaded in wide format from (current USD)
# # https://data.worldbank.org/indicator/NY.GDP.PCAP.CD
# # THEN remove empty headers and save as version 2
# ### CRAP, this includes country aggregates and not just countries. BOOOOOO!
# d <- read_excel("../../Data/WDI/API_NY.GDP.PCAP.CD_DS2_en_excel_v2_130121 v2.xls", na = "NA")
# 
# # Convert wide to long format
# d <- d |>
#   select(-`Indicator Code`, -`Indicator Name`) |>
#   rename(country = `Country Name`, ccode = `Country Code`) |>
#   pivot_longer(cols = `1960`:`2024`, names_to = "year", values_to = "gdp_pc_current_usd")
# 
# # Check for earliest year wide available data
# d |>
#   group_by(year) |>
#   summarize(
#     missing = sum(is.na(gdp_pc_current_usd))
#   ) |> print(n=1000)
...
# Obvious countries being missed in the merge (per my view of the map)
# Venezuela - NO RELIABLE DATA
# South Sudan
# Ethiopia - No data
# Eritrea - No data
# Yemen - No data
...
# Update code in d2024
# worldmap$adm0_a3[worldmap$admin == "Venezuela"]
# d2024$ccode[d2024$country == "Venezuela"]

#worldmap$adm0_a3[worldmap$admin == "South Sudan"]
d2024$ccode[d2024$country == "South Sudan"] <- "SDS"

# worldmap$adm0_a3[worldmap$admin == "Ethiopia"]
# d2024$ccode[d2024$country == "Ethiopia"]

# worldmap$adm0_a3[worldmap$admin == "Eritrea"]
# d2024$ccode[d2024$country == "Eritrea"]

# Use 2023 data for S Korea
# worldmap$adm0_a3[worldmap$admin == "South Korea"]
# d2024$ccode[d2024$country == "Korea, Rep."]
d2024$gdp_pc_current_usd[d2024$country == "Korea, Rep."] <- d2023$gdp_pc_current_usd[d2023$country == "Korea, Rep."]

d10 <- left_join(worldmap, d2024, by = c("adm0_a3" = "ccode"))
...
d10 |>
  mutate(
    gdp_quartiles = case_when(
      gdp_pc_current_usd <= quantile(d2024$gdp_pc_current_usd, probs = .25, na.rm = TRUE) ~ "Quartile 1",
      gdp_pc_current_usd <= quantile(d2024$gdp_pc_current_usd, probs = .5, na.rm = TRUE) ~ "Quartile 2",
      gdp_pc_current_usd <= quantile(d2024$gdp_pc_current_usd, probs = .75, na.rm = TRUE) ~ "Quartile 3",
      gdp_pc_current_usd > quantile(d2024$gdp_pc_current_usd, probs = .75, na.rm = TRUE) ~ "Quartile 4"
    )
  ) |>
  ggplot() +
  geom_sf(aes(fill = gdp_quartiles)) +
  labs(fill = "", title = "GDP per capita (2024 USD)",
...
This is 2024 data from the World Bank on the size of country economies around the world
...
annotate("text", x = 1972, y = 4000, label = "Richest Quartile", color = "darkblue", hjust = 0) +
  coord_cartesian(ylim = c(0, 60000), xlim = c(1970, 2024))
